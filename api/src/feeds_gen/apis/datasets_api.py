...
async def datasets_gtfs_id_get(
    id: str = Path(None, description="The dataset id of the requested dataset."),
    token_ApiKeyAuth: TokenModel = Security(
        get_token_ApiKeyAuth
    ),
) -> Dataset:
    """Get the specified dataset in the Mobility Database."""
    return BaseDatasetsApi.subclasses[0]().datasets_gtfs_id_get(id)
...
async def feeds_gtfs_id_datasets_get(
    id: str = Path(None, description="The feed id of the feed for which to obtain datasets."),
    ...
    return BaseDatasetsApi.subclasses[0]().feeds_gtfs_id_datasets_get(id, latest, limit, offset, filter, sort, bounding_latitudes, bounding_longitudes, bounding_filter_method)

...
async def feeds_get(
    ...
    return BaseFeedsApi.subclasses[0]().feeds_get(limit, offset, filter, sort)
...
async def feeds_gtfs_get(
    ...
    return BaseFeedsApi.subclasses[0]().feeds_gtfs_get(limit, offset, filter, sort, bounding_latitudes, bounding_longitudes, bounding_filter_method)
...
async def feeds_gtfs_id_get(
    id: str = Path(None, description="The feed id of the requested feed."),
    token_ApiKeyAuth: TokenModel = Security(
        get_token_ApiKeyAuth
    ),
) -> GtfsFeed:
    """Get the specified feed from the Mobility Database."""
    return BaseFeedsApi.subclasses[0]().feeds_gtfs_id_get(id)
...
async def feeds_id_get(
    ...
    """Get the specified feed from the Mobility Database."""
    return BaseFeedsApi.subclasses[0]().feeds_id_get(id)

...
class TestFeedReliabilitySummaryFromOrm(unittest.TestCase):
    ...
    def test_probation_cancelled_by_a_failing_criterion(self):
        """A feed failing one criterion is failing the seal, not waiting out a clean run for it."""
        feed = make_feed(
            has_seal=False,
            criteria=[
                make_criterion(SealCriterionName.AVAILABLE, probation_start=NOW - timedelta(days=10)),
                make_criterion(SealCriterionName.COMPLIANT, observed_status="fail", confirmed_status="fail"),
            ],
        )
        result = FeedReliabilitySummaryImpl.from_orm(feed)

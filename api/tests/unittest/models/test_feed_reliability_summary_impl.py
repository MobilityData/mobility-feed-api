...
def make_seal_row(**overrides):
    """A seal roll-up row shaped like a `feedsearch` view row (the `from_orm` search path)."""
    values = {
        "has_seal": True,
...
def make_feed(has_seal=True, seal_lost_at=None, criteria=None):
    """A feed carrying its eager-loaded seal relationships (the `from_feed` path)."""
...
class TestFeedReliabilitySummaryFromOrm(unittest.TestCase):
    """Test `FeedReliabilitySummaryImpl.from_orm` - the `feedsearch` view-row path (search)."""
...
class TestFeedReliabilitySummaryFromFeed(unittest.TestCase):
    """Test `FeedReliabilitySummaryImpl.from_feed` - the ORM-relationship path (detail / list)."""
    ...
    def test_no_seal_row_returns_none(self):
        ...
        assert FeedReliabilitySummaryImpl.from_feed(feed, NOW) is None
    ...
    def test_evaluated_at_is_latest_across_criteria(self):
        ...
        result = FeedReliabilitySummaryImpl.from_feed(feed, NOW)

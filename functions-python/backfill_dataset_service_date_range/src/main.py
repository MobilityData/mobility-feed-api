...
# purpose of this function is to backfill the service_date_range_start and service_date_range_end columns in the gtfsdataset table that are null
...
def backfill_datasets(session: "Session"):
    ...
    datasets = session.query(
        Gtfsdataset
            ...
            or_(Gtfsdataset.service_date_range_start.is_(None),
    ...
    for dataset in datasets:
        ...
        if not latest_validation_report:
            print(f"Skipping gtfsdataset ID {gtfsdataset_id}: no validation reports found.")
            continue
        ...
        try:
            ...
            extracted_service_start_date = json_data.get("summary", {}).get("feedInfo", {}).get("feedStartDate", None)
            extracted_service_end_date = json_data.get("summary", {}).get("feedInfo", {}).get("feedEndDate", None)

            if extracted_service_start_date is None:
                print(f"Key 'summary.feedInfo.feedStartDate' not found in JSON for gtfsdataset ID {gtfsdataset_id}.")
                continue

            if extracted_service_end_date is None:
                print(f"Key 'summary.feedInfo.feedEndDate' not found in JSON for gtfsdataset ID {gtfsdataset_id}.")
                continue

            dataset.service_date_range_start = extracted_service_start_date
            dataset.service_date_range_end = extracted_service_end_date

            print(f"Updated gtfsdataset ID {gtfsdataset_id} with value: {extracted_service_start_date} - {extracted_service_end_date}")
            changes_count += 1

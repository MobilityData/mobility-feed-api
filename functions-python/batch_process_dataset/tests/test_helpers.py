...
import pytest
from unittest.mock import patch, Mock, ANY, mock_open, create_autospec, MagicMock
...
class TestHelpers(unittest.TestCase):
    ...
    def test_create_bucket(self, mock_storage_client):
        ...
        mock_storage_client.return_value.lookup_bucket.assert_called_once_with("test-bucket")
        mock_storage_client.return_value.create_bucket.assert_called_once_with("test-bucket")
    ...
    def test_download_url_content(self, mock_session):
        ...
        mock_session.return_value.get.assert_called_with(url, headers=ANY, verify=False, timeout=120, stream=True)
    ...
    def test_download_and_get_hash(self):
        ...
        with patch('urllib3.PoolManager.request', return_value=mock_response):
            ...
            self.assertEqual(result, expected_hash, msg=f"Hash mismatch: got {result},"
    ...
    def test_download_and_get_hash_auth_type_1(self):
        ...
        with patch('urllib3.PoolManager.request', return_value=mock_response) as mock_request:
            result_hash = download_and_get_hash(url, file_path, 'sha256', 8192, 1, api_key_parameter_name, credentials)

            self.assertEqual(result_hash, expected_hash, msg=f"Hash mismatch: got {result_hash}, but expected {expected_hash}")

            mock_request.assert_called_with('GET', url, preload_content=False,
    ...
    def test_download_and_get_hash_auth_type_2(self):
        ...
        with patch('urllib3.PoolManager.request', return_value=mock_response) as mock_request:
            result_hash = download_and_get_hash(base_url, file_path, 'sha256', 8192, 2, api_key_parameter_name,
                                                credentials)

            self.assertEqual(result_hash, expected_hash, msg=f"Hash mismatch: got {result_hash}, but expected {expected_hash}")

            mock_request.assert_called_with('GET', modified_url, preload_content=False, headers={})
    ...
    def test_download_and_get_hash_exception(self):
        ...
        with patch('urllib3.PoolManager.request', side_effect=Exception("Network error")):

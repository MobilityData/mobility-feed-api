...
def json_schema_to_bigquery(json_schema):
    def convert_field(field):
        ...
        if bq_type == "RECORD":
            subfields = [convert_field(subfield) for subfield in field.get('fields', [])]
...
def filter_json_by_schema(json_schema, input_json):
    def filter_fields(fields, data):
        ...
        for field in fields:
            ...
            field_mode = field.get('mode', 'NULLABLE')  # Default mode is NULLABLE if not specified
                ...
                if field_type == "RECORD" and "fields" in field:
                    if field_mode == "REPEATED":
                        ...
                        filtered_data[field_name] = [
                            filter_fields(field["fields"], item) for item in data[field_name]
                        ]
                    else:
                        # Handle single RECORD
                        filtered_data[field_name] = filter_fields(field["fields"], data[field_name])
                else:

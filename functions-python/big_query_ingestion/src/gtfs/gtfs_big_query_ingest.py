...
from ..common.bq_data_transfer import bucket_name, BiqQueryDataTransfer
from ..common.feeds_locations import get_feeds_locations_map


class BiqQueryDataTransferGTFS(BiqQueryDataTransfer):
    def __init__(self):
        super().__init__()
        current_dir = os.path.dirname(os.path.abspath(__file__))  # Get the directory of the current file
        self.schema_path = os.path.join(current_dir, 'gtfs_schema.json')  # Replace 'schema.json' with your file name
        self.nd_json_path_prefix = 'ndjson/gtfs'
        ...
        for blob in blobs:
            if 'report_' in blob.name and blob.name.endswith('.json'):
                ...
                if ndjson_blob_name in blob_names:
                    logging.warning(f"NDJSON file {ndjson_blob_name} already exists. Skipping...")
                    continue

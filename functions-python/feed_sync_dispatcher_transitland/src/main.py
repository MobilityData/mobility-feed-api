...
class TransitFeedSyncProcessor(FeedSyncProcessor):
    def process_sync(self, session: Session) -> list[FeedSyncPayload]:
        """
    ...
    def publish_callback(
        ...
        if future.exception():
            print(
                f"Error publishing feed {payload.external_id} to Pub/Sub topic {topic_path}: {future.exception()}"
            )
        else:
            print(f"Published stable_id={payload.external_id}.")

...
from database_gen.sqlacodegen_models import Feed, Externalid, Redirectingid, Location
...
# Configure logging
logger = Logger("feed_processor").get_logger()
logger.addFilter(StableIdFilter())
...
class FeedProcessor:
    ...
    def process_feed(self, payload: FeedPayload) -> None:
        ...
        """
        logger.addFilter(StableIdFilter(payload.external_id))
        try:
            logger.info(
            ...
            if current_feed_id is None:
                ...
                if self.check_feed_url_exists(payload.feed_url):
                    logger.error(f"Feed URL already exists: {payload.feed_url}")
                    return
    ...
    def process_new_feed(self, payload: FeedPayload) -> None:
        ...
        try:
            ...
            if location:
                ...
                logger.debug(f"Added location information for feed: {feed_id}")
    ...
    def process_feed_update(self, payload: FeedPayload, old_feed_id: str) -> None:
        ...
        try:
            ...
            stable_id = (
                ...
                else f"{payload.source}-{payload.external_id}_id-{reference_count + 1}"
            )
            
            logger.debug(f"Generated new stable_id: {stable_id} (reference count: {reference_count})")
    ...
    def check_feed_url_exists(self, feed_url: str) -> bool:
        ...
        if result is not None:
            if result.status == "deprecated":
                ...
                logger.debug(
                    f"Found existing feed with URL: {feed_url} (status: {result.status})"
                )
            return True
    ...
    def get_current_feed_info(
        ...
        if result is not None:
            logger.info(
                f"Retrieved feed {result.stable_id} info for external_id: {external_id} (status: {result.status})"
            )
    ...
    def publish_to_batch_topic(self, payload: FeedPayload) -> None:
        ...
        try:
            ...
            future.result()
            logger.info(f"Published feed {payload.feed_id} to dataset batch topic")
        except Exception as e:

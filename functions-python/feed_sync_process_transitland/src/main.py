...
class FeedProcessor:
    ...
    def process_feed(self, payload: FeedPayload) -> None:
        ...
        try:
            log_message("info", f"Starting feed processing for external_id: {payload.external_id}")
            ...
            if current_feed_id is None:
                ...
                if current_url != payload.feed_url:
                    ...
                    log_message("debug", f"Found existing feed: {current_feed_id} with different URL")
                    self.process_feed_update(payload, current_feed_id)
                else:
                    log_message("error", f"Feed already exists with same URL: {payload.external_id}")
                    return
            ...
            error_msg = f"Database error processing feed {payload.external_id}: {str(e)}"
    ...
    def process_new_feed(self, payload: FeedPayload) -> None:
        ...
        """
        log_message("info", f"Starting new feed creation for external_id: {payload.external_id}")
        ...
        log_message("debug", f"Generated new feed_id: {feed_id} and stable_id: {stable_id}")
        ...
        try:
            ...
            log_message("info", f"Created new feed with ID: {feed_id} for external_id: {payload.external_id}")

        except Exception as e:
            log_message("error", f"Error creating new feed for external_id {payload.external_id}: {str(e)}")
            raise
    ...
    def process_feed_update(self, payload: FeedPayload, old_feed_id: str) -> None:
        ...
        """
        log_message("info", f"Starting feed update process for external_id: {payload.external_id}")
        ...
        try:
            ...
            log_message("debug", f"Generated new stable_id: {stable_id} (reference count: {reference_count})")
            ...
            log_message("debug", f"Created new external ID mapping for feed_id: {new_feed_id}")

            # Create redirect
            redirect = Redirectingid(source_id=old_feed_id, target_id=new_feed_id)
            self.session.add(redirect)
            log_message("debug", f"Created redirect from {old_feed_id} to {new_feed_id}")
            ...
            if location:
                new_feed.locations.append(location)
                log_message("debug", f"Added location information for feed: {new_feed_id}")

            self.session.flush()

            log_message("info", f"Updated feed for external_id: {payload.external_id}, new feed_id: {new_feed_id}")

        except Exception as e:
            log_message("error", f"Error updating feed for external_id {payload.external_id}: {str(e)}")
            raise
    ...
    def check_feed_url_exists(self, feed_url: str) -> bool:
        ...
        result = (
            self.session.query(Feed)
            .filter(Feed.producer_url == feed_url)
            .first()
        )

        if result is not None:
            if result.status == "active":
                log_message("info", f"Found existing feed with URL: {feed_url} (status: active)")
            else:
                log_message("error", f"Feed URL {feed_url} exists in deprecated feed (id: {result.id}). "
    ...
    def get_current_feed_info(
        ...
        if result is not None:
            log_message("info", f"Retrieved feed {result.stable_id} "
    ...
    def publish_to_batch_topic(self, payload: FeedPayload) -> None:
        ...
        message_data = {
            ...
            "dataset_id": None,  # This will be generated by the dataset processor
            "dataset_hash": None,  # This will be calculated by the dataset processor
            "authentication_type": payload.type if payload.type else "0",
            "authentication_info_url": payload.auth_info_url,
            "api_key_parameter_name": payload.auth_param_name
        }
        ...
        try:
            ...
            future.result()
            log_message("info", f"Published feed {payload.feed_id} to dataset batch topic")
        except Exception as e:

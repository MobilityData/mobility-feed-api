...
async def test_update_gtfs_rt_feed_set_wip(
    ...
    update_request_gtfs_rt_feed.operational_status = "wip"
    api = OperationsApiImpl()
    response: Response = api.update_gtfs_rt_feed(update_request_gtfs_rt_feed)
    assert response.status_code == 200
...
async def test_update_gtfs_rt_feed_set_published(
    ...
    update_request_gtfs_rt_feed.operational_status = "published"
    api = OperationsApiImpl()
    response: Response = api.update_gtfs_rt_feed(update_request_gtfs_rt_feed)
    assert response.status_code == 200
...
async def test_update_gtfs_rt_feed_set_unpublished(
    ...
    update_request_gtfs_rt_feed.operational_status = "unpublished"
    api = OperationsApiImpl()
    response: Response = api.update_gtfs_rt_feed(update_request_gtfs_rt_feed)
    assert response.status_code == 200
...
@pytest.mark.asyncio
@patch("feeds_operations.impl.feeds_operations_impl.create_web_revalidation_task")
async def test_update_gtfs_rt_feed_seasonal_field(
    mock_revalidation, update_request_gtfs_rt_feed, db_session
):
    ...
    update_request_gtfs_rt_feed.seasonal = True
    api = OperationsApiImpl()
    response: Response = api.update_gtfs_rt_feed(update_request_gtfs_rt_feed)
    assert response.status_code == 200
...
async def test_update_gtfs_rt_feed_omitted_seasonal_is_preserved(
    ...
    assert update_request_gtfs_rt_feed.seasonal is None
    ...
    assert db_feed.note == feed_mdb_41.note
    assert db_feed.seasonal is True

...
async def test_get_license_success(db_session):
    ...
    # Act
    result = api.get_license(existing.id)
...
async def test_get_license_not_found():
    ...
    with pytest.raises(HTTPException) as exc_info:
        api.get_license("non-existent-license-id")
...
async def test_get_licenses_basic_pagination(db_session):
    ...
    # Act
    result = api.get_licenses(limit=10, offset=0)
...
async def test_get_licenses_with_search_query(db_session):
    ...
    result = api.get_licenses(limit=50, offset=0, search_query=fragment)
...
async def test_get_licenses_filter_is_spdx_true(db_session):
    ...
    api = LicensesApiImpl()
    result = api.get_licenses(limit=10, offset=0, is_spdx=True)
...
async def test_get_licenses_filter_is_spdx_false(db_session):
    ...
    api = LicensesApiImpl()
    result = api.get_licenses(limit=10, offset=0, is_spdx=False)
...
async def test_get_license_includes_rules(db_session):
    ...
    api = LicensesApiImpl()
    result = api.get_license("custom-test")
...
async def test_get_licenses_includes_rules_for_each_item(db_session):
    ...
    api = LicensesApiImpl()
    results = api.get_licenses(limit=10, offset=0)

    assert results
    for item in results:
        if item.id == "MIT":
            detailed = api.get_license("MIT")
...
async def test_get_matching_licenses_success(monkeypatch, db_session):
    ...
    # Act
    result = api.get_matching_licenses(request)
...
async def test_get_matching_licenses_propagates_http_exception(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        api.get_matching_licenses(request)

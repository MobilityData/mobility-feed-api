...
class TestBuildContexts(SealDbTestCase):
    ...
    def test_freshs_inputs_reach_the_context_through_its_own_loader(self, db_session):
        ...
        Fresh's inputs are not a field on the context — its latest dataset is a different row
        on each day a march evaluates — so they arrive under the criterion's own key and it
...
class TestFreshCoverageLoadInputs(SealDbTestCase):
    ...
    @with_db_session(db_url=default_db_url)
    def test_the_latest_dataset_is_resolved_as_of_the_day_asked_for(self, db_session):

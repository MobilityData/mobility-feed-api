...
class GTFSDatasetsEndpointTests(IntegrationTests):
    ...
    def test_all_datasets(self):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            task = self.progress.add_task("[yellow]Validating the feeds' latest datasets...[/yellow]",
                                          total=len(feed_ids))

            # Map each future to its feed ID
            future_to_feed_id = {executor.submit(self.validate_feed, feed_id): feed_id for feed_id in feed_ids}
            ...
            for future in as_completed(future_to_feed_id):
                ...
                if error_entry is not None:
                    ...
                    self.console.log(
                        f"Feed '{error_entry['stable_id']}' has a related [red]error[/red]: {error_entry['Error Details']}")
    ...
    def validate_feed(self, feed_id):
        ...
        try:
            ...
            # Validate datasets
            response = self.get_response(f'v1/gtfs_feeds/{stable_id}/datasets', params={'latest': True})
    ...
    def _validate_feed_response(response):
        if response.status_code != 200:
            error_code = FeedError.NOT_FOUND.name if response.status_code == 404 else FeedError.API_ERROR.name
            error_detail = FeedError.NOT_FOUND.value if response.status_code == 404 else FeedError.API_ERROR.value
    ...
    def _validate_dataset(datasets, status_code):
        ...
        try:
            ...
            raise Exception(f"{FeedError.INVALID_DATASET.name}: {FeedError.INVALID_DATASET.value} -- {e}")
    ...
    def _log_errors(self, errors):
        table = Table(title="Test Errors", show_header=True, header_style="bold magenta", show_lines=True, expand=True)
        table.add_column("Stable ID", style="dim")
        table.add_column("Error Code", style="red")
        table.add_column("Error Details", overflow="fold")
        for error in errors:
            table.add_row(error['stable_id'], error['Error Code'], error['Error Details'])
        self.console.print(table)
        pandas.DataFrame(errors).to_csv('datasets_validation.csv')
        self.console.log('Datasets validation report saved to "datasets_validation.csv"')

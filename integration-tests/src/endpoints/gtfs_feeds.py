...
class GTFSFeedsEndpointTests(IntegrationTests):
    ...
    def test_gtfs_feeds(self):
        ...
        assert response.status_code == 200, "Expected 200 status code for GTFS feeds, got {response.status_code}."
        gtfs_feeds = response.json()
        task_id = self.progress.add_task("[yellow]Validating GTFS feeds type...[/yellow]", total=len(gtfs_feeds))
        for i, feed in enumerate(gtfs_feeds):
            assert feed['data_type'] == 'gtfs', f"Expected data_type 'gtfs', got '{feed['data_type']}'."
            self.progress.update(task_id, advance=1, description=f"Validated GTFS feed {feed['id']} "
    ...
    def test_filter_by_country_code_gtfs(self):
        ...
        task_id = self.progress.add_task("[yellow]Validating GTFS feeds by country code...[/yellow]", len(country_codes)
                                         )
    ...
    def test_filter_by_provider_gtfs(self):
        ...
        task_id = self.progress.add_task("[yellow]Validating GTFS feeds by provider...[/yellow]", total=len(providers))
        for i, provider_id in enumerate(providers):
            self._test_filter_by_provider(provider_id, 'v1/gtfs_feeds',
                                          task_id=task_id,
    ...
    def test_filter_by_municipality_gtfs(self):
        ...
        task_id = self.progress.add_task("[yellow]Validating GTFS feeds by municipality...[/yellow]",
                                         total=len(municipalities))
        for i, municipality in enumerate(municipalities):
            self._test_filter_by_municipality(municipality, 'v1/gtfs_feeds', validate_location=True,
                                              task_id=task_id, index=f'{i + 1}/{len(municipalities)}')

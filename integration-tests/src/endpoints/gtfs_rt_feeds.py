...
class GTFSRTFeedsEndpointTests(IntegrationTests):
    ...
    def test_filter_by_provider_gtfs_rt(self):
        ...
        task_id = self.progress.add_task("[yellow]Validating GTFS Realtime feeds by provider...[/yellow]",
                                         total=len(providers))

        for i, provider_id in enumerate(providers):
            self._test_filter_by_provider(provider_id, 'v1/gtfs_rt_feeds', task_id=task_id,
    ...
    def test_filter_by_country_code_gtfs_rt(self):
        ...
        task_id = self.progress.add_task("[yellow]Validating GTFS Realtime feeds by country code...[/yellow]",
                                         total=len(country_codes))

        for i, country_code in enumerate(country_codes):
            self._test_filter_by_country_code(country_code, 'v1/gtfs_rt_feeds', task_id=task_id,
    ...
    def test_filter_by_municipality_gtfs_rt(self):
        ...
        task_id = self.progress.add_task("[yellow]Validating GTFS Realtime feeds by municipality...[/yellow]",
                                         total=len(municipalities))
        for i, municipality in enumerate(municipalities):
            self._test_filter_by_municipality(municipality, 'v1/gtfs_rt_feeds', task_id=task_id,

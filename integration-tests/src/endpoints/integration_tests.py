...
class IntegrationTests:
    ...
    def _test_filter_by_country_code(self, country_code, endpoint, validate_location=False, task_id=None, index='1/1'):
        ...
        assert response.status_code == 200, (f"Expected 200 status code for country code filter '{country_code}', got "
                                             f"{response.status_code}.")
        feeds = response.json()
        assert len(feeds) > 0, f"Expected at least one feed with country code '{country_code}', got 0."
        if validate_location:
            for feed in feeds:
                country_codes = [location['country_code'] for location in feed['locations']]
                assert country_code in country_codes, (
                    f"Expected all feeds to have country code '{country_code}', but found country codes "
                    f"'{country_codes}'"
                )
        self._update_progression(task_id, f"Validated country code {country_code}", index)
    ...
    def _update_progression(self, task_id, validation_description, index):
        ...
        if self.progress is not None and task_id is not None:
            self.progress.update(task_id, advance=1, description=f"{validation_description} ({index})")
    ...
    def _test_filter_by_provider(self, provider, endpoint, task_id, index):
        ...
        assert response.status_code == 200, (f"Expected 200 status code for provider filter '{provider}', got "
                                             f"{response.status_code}.")
        feeds = response.json()
        assert len(feeds) > 0, f"Expected at least one feed with provider '{provider}', got 0."
        for feed in feeds:
            assert provider.lower() in feed.get('provider').lower(), (
    ...
    def _test_filter_by_municipality(self, municipality, endpoint, validate_location=False, task_id=None, index='1/1'):
        ...
        assert response.status_code == 200, (f"Expected 200 status code for municipality filter '{municipality}', got "
                                             f"{response.status_code}.")
        feeds = response.json()
        assert len(feeds) > 0, f"Expected at least one feed with municipality '{municipality}', got 0."
        if validate_location:
            for feed in feeds:
                municipalities = [location['municipality'] for location in feed['locations']]
        ...
        self._update_progression(task_id, f"Validated municipality {municipality}", index)
    ...
    def get_test_methods_for_class(cls):
        ...
        return [method_name for method_name in dir(cls)
                if
                callable(getattr(cls, method_name)) and method_name.startswith('test_') and method_name != "test_all"]
    ...
    def test_all(self, target_classes=[]):
        ...
        console.print(Panel(description, title="[bold green]API Integration Test Suite[/bold green]",
        ...
        with Progress(
            ...
            for child_class, methods in test_methods:
                try:
                    instance = child_class(self.file_path, self.access_token, self.base_url, progress, self.console)
                except TypeError:
                    instance = child_class(self.file_path, self.access_token, self.base_url, progress)
                ...
                for method_name in methods:
                    ...
                    try:
                        ...
                        progress.log(f"{method_display} [green]PASSED :white_check_mark: [/green] in [yellow]"
                                     f"{(time.time() - start_time):.2f}s[/yellow]")
                        passed_tests.append(method_display)
                    except AssertionError:
                        progress.log(f"{method_display} [red]FAILED :cross_mark: [/red] in [yellow]"
                                     f"{(time.time() - start_time):.2f}s[/yellow]")
                        progress.console.print_exception(show_locals=True, suppress=["endpoints/integration_tests.py"],
                                                         max_frames=0)
                        failed_tests.append(method_display)
                    except Exception:
                        progress.log(f"{method_display} [yellow]SKIPPED :warning: [/yellow] in [yellow]"
                                     f"{(time.time() - start_time):.2f}s[/yellow]")
                        progress.console.print_exception(show_locals=True, suppress=["endpoints/integration_tests.py"],
                                                         max_frames=0)
        ...
        table = Table(title="[bold cyan]TEST REPORT[/bold cyan]", show_lines=True, expand=True)

        table.add_column("Status", justify="center", vertical='middle')
        table.add_column("Nb Of Tests", style="yellow", justify='center', vertical='middle')
        table.add_column("Tests List", justify="left", overflow='fold', vertical='middle')

        table.add_row("[green]PASSED[/green]", f"{len(passed_tests)}", ', '.join(passed_tests))
        table.add_row("[red]FAILED[/red]", f'{len(failed_tests)}', ', '.join(failed_tests))
        table.add_row("[yellow]SKIPPED[/yellow]", f'{len(skipped_tests)}', ', '.join(skipped_tests))

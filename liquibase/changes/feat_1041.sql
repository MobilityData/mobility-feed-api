...
CREATE TABLE GBFSVersion(
    ...
    feed_id VARCHAR(255) NOT NULL REFERENCES GBFSFeed(id), -- Link to the feed that this version belongs to
    version VARCHAR(6) NOT NULL,
    url TEXT NOT NULL,
    latest BOOLEAN NOT NULL DEFAULT FALSE, -- Whether the version is the latest or not
    is_active BOOLEAN NOT NULL DEFAULT TRUE, -- Whether the version is still active or not
...
CREATE TABLE GBFSEndpoint(
    id VARCHAR(255) NOT NULL PRIMARY KEY,
    gbfs_version_id VARCHAR(255) NOT NULL REFERENCES GBFSVersion(id), -- Link to the version that this endpoint belongs to
    ...
    is_active BOOLEAN NOT NULL DEFAULT TRUE -- Whether the endpoint is still active or not
);

-- 2.3 Recreating the GBFSValidationReport table
CREATE TABLE GBFSValidationReport(
    id VARCHAR(255) NOT NULL PRIMARY KEY,
    gbfs_version_id VARCHAR(255) NOT NULL REFERENCES GBFSVersion(id), -- Link to the version that this report belongs to
...
CREATE TABLE GBFSNotice(
    ...
    validation_report_id VARCHAR(255) NOT NULL REFERENCES GBFSValidationReport(id),
...
CREATE TABLE IF NOT EXISTS GBFSFeedHTTPAccessLog (
    http_access_log UUID NOT NULL REFERENCES HTTPAccessLog(id),
    gbfs_feed_id VARCHAR(255) NOT NULL REFERENCES GBFSFeed(id),
...
CREATE TABLE IF NOT EXISTS GBFSEndpointHTTPAccessLog (
    http_access_log UUID NOT NULL REFERENCES HTTPAccessLog(id),
    gbfs_endpoint_id VARCHAR(255) NOT NULL REFERENCES GBFSEndpoint(id),

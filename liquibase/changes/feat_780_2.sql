DO $$
BEGIN
    -- Check if the column 'internal_status' exists in the 'Feed' table
    IF EXISTS (
        SELECT 1
        ...
        -- Rename 'internal_status' column to 'operational_status'
        ALTER TABLE Feed RENAME COLUMN internal_status TO operational_status;
    END IF;
END $$;

DO $$
BEGIN
    -- Check if the type 'InternalStatus' exists
    IF EXISTS (
        SELECT 1
        FROM pg_type
        WHERE typname = 'internalstatus'
    ) THEN
        -- Rename the type 'InternalStatus' to 'OperationalStatus'
        ALTER TYPE InternalStatus RENAME TO OperationalStatus;
    END IF;
END $$;

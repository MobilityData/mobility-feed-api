...
SERVICE_ACCOUNT=functions-service-account@mobility-feeds-dev.iam.gserviceaccount.com
ENVIRONMENT=dev
...
ENV_VARS=$(jq -r '.environment_variables | to_entries | map("--set-env-vars \(.key)=\(.value)") | join(" ") // empty' "$CONFIG_FILE")
...
for SECRET_NAME in $SECRETS; do
  gcloud secrets add-iam-policy-binding DEV_$SECRET_NAME \
    --project $PROJECT \
...
# Deploy the function
DEPLOY_CMD="gcloud functions deploy $FUNCTION_NAME --gen2 
          --project $PROJECT --region northamerica-northeast1 
          --runtime $RUNTIME --entry-point $ENTRY_POINT 
          --source $SOURCE --service-account $SERVICE_ACCOUNT"

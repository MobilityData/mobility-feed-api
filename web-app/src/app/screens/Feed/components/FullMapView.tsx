import { Box, Fab, Button, Chip, useTheme, CircularProgress } from '@mui/material';
...
import { loadingRoutes } from '../../../store/route-reducer';
...
export default function FullMapView(): React.ReactElement {
  ...
  useEffect(() => {
    if (feedId && latestDataset?.stable_id) {
      dispatch(
        loadingRoutes({ feedId, datasetId: latestDataset.stable_id }),
      );
    }
  }, [feedId, latestDataset, dispatch]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {routesLoading && <CircularProgress />}
        {routesError && <p>Error loading routes.</p>}
        {routes && (
          <NestedCheckboxList
            checkboxData={getUniqueRouteTypesCheckboxData(routes)}
      ...
      >
        ...
        >
          ...
          {boundingBox && feedId && latestDataset && (
            <GtfsVisualizationMap
              ...
              hideStops={hideStops}
              feedStableId={feedId}
              datasetStableId={latestDataset.stable_id}
            ></GtfsVisualizationMap>
          )}
        </Box>
      </Box>
